...
if(APPLE)
  ...
  endforeach()
  if(IOS)
    set(_fonts_mpq "${CMAKE_CURRENT_SOURCE_DIR}/Packaging/ios/vendor/fonts.mpq")
    set(_fonts_url "https://github.com/diasurgical/devilutionx-assets/releases/download/v5/fonts.mpq")
    if(NOT EXISTS "${_fonts_mpq}")
      message(STATUS "Downloading extra CJK fonts for iOS")
      file(MAKE_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}/Packaging/ios/vendor")
      file(DOWNLOAD "${_fonts_url}" "${_fonts_mpq}" SHOW_PROGRESS TLS_VERIFY ON STATUS _fonts_status)
      list(GET _fonts_status 0 _fonts_code)
      if(NOT _fonts_code EQUAL 0)
        list(GET _fonts_status 1 _fonts_msg)
        file(REMOVE "${_fonts_mpq}")
        message(WARNING "Could not download fonts.mpq (${_fonts_msg}); in-game Chinese UI needs fonts.mpq")
      endif()
    endif()
    if(EXISTS "${_fonts_mpq}")
      set_source_files_properties("${_fonts_mpq}" PROPERTIES
        MACOSX_PACKAGE_LOCATION Resources
        XCODE_EXPLICIT_FILE_TYPE compiled)
      target_sources(${BIN_TARGET} PRIVATE "${_fonts_mpq}")
    endif()
  endif()
else()
