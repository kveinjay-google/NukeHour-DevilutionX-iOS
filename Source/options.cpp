...
void LoadOptions()
{
	...
	options.Controller.virtualGamepadCustomLayout = ini->getBool("Controller", "Custom Layout", false);
	options.Controller.virtualGamepadPadX = ini->getFloat("Controller", "Pad X", 0.0F);
	options.Controller.virtualGamepadPadY = ini->getFloat("Controller", "Pad Y", 0.0F);
	options.Controller.virtualGamepadActionsX = ini->getFloat("Controller", "Actions X", 0.0F);
	options.Controller.virtualGamepadActionsY = ini->getFloat("Controller", "Actions Y", 0.0F);
	options.Controller.virtualGamepadMenuX = ini->getFloat("Controller", "Menu X", 0.0F);
	options.Controller.virtualGamepadMenuY = ini->getFloat("Controller", "Menu Y", 0.0F);
#ifdef __vita__
...
void SaveOptions()
{
	...
	ini->set("Controller", "Custom Layout", options.Controller.virtualGamepadCustomLayout);
	ini->set("Controller", "Pad X", options.Controller.virtualGamepadPadX);
	ini->set("Controller", "Pad Y", options.Controller.virtualGamepadPadY);
	ini->set("Controller", "Actions X", options.Controller.virtualGamepadActionsX);
	ini->set("Controller", "Actions Y", options.Controller.virtualGamepadActionsY);
	ini->set("Controller", "Menu X", options.Controller.virtualGamepadMenuX);
	ini->set("Controller", "Menu Y", options.Controller.virtualGamepadMenuY);
#ifdef __vita__
...
#ifdef __IPHONEOS__
	// 640x480 makes the original 128px HUD fill too much of the screen and
	// collide with the virtual stick. Prefer a taller landscape canvas.
	if (size_.width <= 640 && size_.height <= 480) {
		size_ = { 960, 640 };
	}
#endif
}
...
GameplayOptions::GameplayOptions()
    ...
    , runInTown("Run in Town", OptionEntryFlags::CantChangeInMultiPlayer, N_("Run in Town"), N_("Enable jogging/fast walking in town for Diablo and Hellfire. This option was introduced in the expansion."),
#ifdef __IPHONEOS__
          true
#else
          false
    ...
    , pauseOnFocusLoss("Pause Game When Window Loses Focus", OptionEntryFlags::None, N_("Pause Game When Window Loses Focus"), N_("When enabled, the game will pause when focus is lost."),
#ifdef __IPHONEOS__
          false
#else
          true
    ...
    , enemyHealthBar("Enemy Health Bar", OptionEntryFlags::None, N_("Enemy Health Bar"), N_("Enemy Health Bar is displayed at the top of the screen."),
#ifdef __IPHONEOS__
          true
#else
          false
    ...
    , showMonsterType("Show Monster Type", OptionEntryFlags::None, N_("Show Monster Type"), N_("Hovering over a monster will display the type of monster in the description box in the UI."),
#ifdef __IPHONEOS__
          true
#else
          false
    ...
    , showItemLabels("Show Item Labels", OptionEntryFlags::None, N_("Show Item Labels"), N_("Show labels for items on the ground when enabled."),
#ifdef __IPHONEOS__
          true
#else
          false
...
#endif
          N_("Virtual Gamepad"),
          N_("When to show the on-screen controls. Auto hides them while a gamepad is in use."),
#if defined(__IPHONEOS__) && TARGET_OS_SIMULATOR
          VirtualGamepadVisibility::Always,
#else
          VirtualGamepadVisibility::Auto,
#endif
          {
              { VirtualGamepadVisibility::Auto, N_("Auto") },
              { VirtualGamepadVisibility::Always, N_("Always") },
              { VirtualGamepadVisibility::Hidden, N_("Hidden") },
          })
{
}
...
void OptionEntryLanguageCode::LoadFromIni(std::string_view category)
{
...
#ifdef __IPHONEOS__
	if (!HaveExtraFonts()) {
		const std::string_view code = szCode;
		if (code == "zh_CN" || code == "zh_TW" || code == "ja" || code == "ko" || code == "he") {
			CopyUtf8(szCode, "en", sizeof(szCode));
		}
	} else if (szCode[0] == '\0' || std::string_view(szCode) == "en") {
		// Previous iOS builds forced English when fonts.mpq was missing.
		const std::vector<std::string> locales = GetLocales();
		bool prefersZhTW = false;
		for (const auto &locale : locales) {
			if (locale == "zh_TW" || locale.rfind("zh_Hant", 0) == 0) {
				prefersZhTW = true;
				break;
			}
		}
		CopyUtf8(szCode, prefersZhTW ? "zh_TW" : "zh_CN", sizeof(szCode));
	}
#endif
	...
	for (const auto &locale : locales) {
#ifdef __IPHONEOS__
		if (!HaveExtraFonts() && (locale == "zh_CN" || locale == "zh_TW" || locale == "ja" || locale == "ko" || locale == "he" || locale.rfind("zh", 0) == 0)) {
			continue;
		}
#endif
