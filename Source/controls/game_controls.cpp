...
void ShowMenuButtonHint(std::string_view text, const VirtualMenuButton &button)
{
	const Rectangle &panel = VirtualGamepadState.menuPanel.area;
	ShowVirtualGamepadHint(text, {
	    button.area.position.x + button.area.size.width / 2,
	    panel.position.y + panel.size.height + 8,
	});
}
	...
	if (ControlMode == ControlTypes::VirtualGamepad) {
		...
		case SDL_FINGERDOWN:
			...
			if (VirtualGamepadState.menuPanel.charButton.isHeld && VirtualGamepadState.menuPanel.charButton.didStateChange) {
				ShowMenuButtonHint(_("Character"), VirtualGamepadState.menuPanel.charButton);
				*action = GameAction(GameActionType_TOGGLE_CHARACTER_INFO);
				return true;
			}
			if (VirtualGamepadState.menuPanel.questsButton.isHeld && VirtualGamepadState.menuPanel.questsButton.didStateChange) {
				ShowMenuButtonHint(_("Quests log"), VirtualGamepadState.menuPanel.questsButton);
			...
			if (VirtualGamepadState.menuPanel.inventoryButton.isHeld && VirtualGamepadState.menuPanel.inventoryButton.didStateChange) {
				ShowMenuButtonHint(_("Inventory"), VirtualGamepadState.menuPanel.inventoryButton);
				*action = GameAction(GameActionType_TOGGLE_INVENTORY);
				return true;
			}
			if (VirtualGamepadState.menuPanel.mapButton.isHeld && VirtualGamepadState.menuPanel.mapButton.didStateChange) {
				ShowMenuButtonHint(_("Automap"), VirtualGamepadState.menuPanel.mapButton);
				DoAutoMap();
				*action = GameAction(GameActionType_NONE);
				return true;
			}
			if (VirtualGamepadState.menuPanel.labelsButton.isHeld && VirtualGamepadState.menuPanel.labelsButton.didStateChange) {
				ShowMenuButtonHint(_("Show Item Labels"), VirtualGamepadState.menuPanel.labelsButton);
