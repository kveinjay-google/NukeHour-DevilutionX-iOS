...
void HandleTouchEvent(const SDL_Event &event)
{
	...
	if (IsVirtualGamepadLayoutEditorOpen()) {
		HandleVirtualGamepadLayoutEditorEvent(event);
		return;
	}

#ifdef __IPHONEOS__
	if (gbRunGame && PauseMode == 2 && !gmenu_is_active() && IsFingerDown(event)) {
		diablo_pause_game();
		return;
	}
#endif
...
bool VirtualGamepadEventHandler::HandleMenuPanelEvent(const SDL_Event &event)
{
	if (IsFingerDown(event)) {
		const Point touchCoordinates = ScaleToScreenCoordinates(event.tfinger.x, event.tfinger.y);
		struct MenuCandidate {
			VirtualButtonEventHandler *handler;
			VirtualMenuButton *button;
		};
		const MenuCandidate candidates[] = {
			{ &labelsMenuButtonEventHandler, &VirtualGamepadState.menuPanel.labelsButton },
			{ &charMenuButtonEventHandler, &VirtualGamepadState.menuPanel.charButton },
			{ &questsMenuButtonEventHandler, &VirtualGamepadState.menuPanel.questsButton },
			{ &inventoryMenuButtonEventHandler, &VirtualGamepadState.menuPanel.inventoryButton },
			{ &mapMenuButtonEventHandler, &VirtualGamepadState.menuPanel.mapButton },
		};

		VirtualButtonEventHandler *best = nullptr;
		int64_t bestDist = std::numeric_limits<int64_t>::max();
		for (const MenuCandidate &candidate : candidates) {
			if (!candidate.button->contains(touchCoordinates))
				continue;
			const Point center = candidate.button->area.Center();
			const int64_t dx = static_cast<int64_t>(touchCoordinates.x) - center.x;
			const int64_t dy = static_cast<int64_t>(touchCoordinates.y) - center.y;
			const int64_t dist = dx * dx + dy * dy;
			if (dist < bestDist) {
				bestDist = dist;
				best = candidate.handler;
			}
		}
		if (best != nullptr)
			return best->Handle(event);
		return false;
	}
	...
	if (mapMenuButtonEventHandler.Handle(event))
		return true;
	return labelsMenuButtonEventHandler.Handle(event);
}
...
bool VirtualDirectionPadEventHandler::HandleFingerDown(const SDL_TouchFingerEvent &event)
{
	...
	if (!virtualDirectionPad->canActivate(touchCoordinates))
		return false;

	virtualDirectionPad->BeginTouch(touchCoordinates);
...
bool VirtualDirectionPadEventHandler::HandleFingerUp(const SDL_TouchFingerEvent &event)
{
	...
	virtualDirectionPad->Deactivate();
	isActive = false;
	return true;
}
