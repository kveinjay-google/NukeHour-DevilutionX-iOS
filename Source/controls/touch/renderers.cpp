...
{
	if (!gbRunGame && !IsVirtualGamepadLayoutEditorOpen())
		return;
...
void VirtualGamepadRenderer::Render(RenderFunction renderFunction)
{
	...
	if (IsVirtualGamepadLayoutEditorOpen() || leveltype != DTYPE_TOWN)
...
void VirtualMenuPanelRenderer::Render(RenderFunction renderFunction)
{
	...
	const bool hasStatPts = MyPlayer != nullptr && MyPlayer->_pStatPts != 0;
	renderFunction(hasStatPts ? menuArtLevelUp : menuArt, nullptr, &rect);
}
