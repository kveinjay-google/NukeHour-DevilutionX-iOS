...
bool IsRunInTownEnabled()
{
	if (gbIsMultiplayer)
		return sgGameInitInfo.bRunInTown != 0;
	return *GetOptions().Gameplay.runInTown;
}
...
void StartWalkAnimation(Player &player, Direction dir, bool pmWillBeCalled)
{
	...
	if (leveltype == DTYPE_TOWN && IsRunInTownEnabled())
		skippedFrames = 2;
	if (pmWillBeCalled)
		skippedFrames += 1;
...
bool DoWalk(Player &player)
{
	...
	if (*GetOptions().Audio.walkingSound && (leveltype != DTYPE_TOWN || !IsRunInTownEnabled())) {
