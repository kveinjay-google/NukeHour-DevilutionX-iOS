...
#endif
#ifdef __IPHONEOS__
#include "platform/ios/ios_file_import.h"
#endif
...
#ifdef __IPHONEOS__
		if (vecItem->m_value == static_cast<int>(SpecialMenuEntry::ImportGameData)) {
			auto paragraphs = WordWrapString(_("Import a legally owned DIABDAT.MPQ, spawn.mpq, or Hellfire archives. Completely quit and reopen after adding Hellfire."), rectDescription.size.width, GameFont12, 1);
			CopyUtf8(optionDescription, paragraphs, sizeof(optionDescription));
			return;
		}
#endif
...
#ifndef USE_SDL1
		case SpecialMenuEntry::CustomizeGamepad:
			RunVirtualGamepadLayoutEditor();
			break;
		case SpecialMenuEntry::ResetGamepad:
			ResetVirtualGamepadLayout();
			SaveOptions();
			break;
#endif
		case SpecialMenuEntry::ImportGameData:
#ifdef __IPHONEOS__
			IOSPresentImportScreen(IOSImportGateOptional);
#endif
			break;
		}
		return;
...
void UiSettingsMenu()
{
	...
	do {
		...
		vecDialog.push_back(std::make_unique<UiArtText>(titleText.data(), MakeSdlRect(uiRectangle.position.x + 150, uiRectangle.position.y + 161, uiRectangle.size.width - 300, 35), UiFlags::FontSize30 | UiFlags::ColorUiSilver | UiFlags::AlignCenter, 8));
		vecDialog.push_back(std::make_unique<UiArtTextButton>(_("Back"), &UiFocusNavigationEsc,
		    MakeSdlRect(uiRectangle.position.x + 16, uiRectangle.position.y + 155, 140, 48),
		    UiFlags::VerticalCenter | UiFlags::FontSize30 | UiFlags::ColorUiGold));
		vecDialog.push_back(std::make_unique<UiArtTextButton>(_("Select"), &UiFocusNavigationSelect,
		    MakeSdlRect(uiRectangle.position.x + uiRectangle.size.width - 156, uiRectangle.position.y + 155, 140, 48),
		...
		case ShownMenuType::Categories: {
#ifdef __IPHONEOS__
			vecDialogItems.push_back(std::make_unique<UiListItem>(_("Import Game Data"), static_cast<int>(SpecialMenuEntry::ImportGameData), UiFlags::ColorUiGold));
#endif
			size_t catIndex = 0;
		...
		vecDialogItems.push_back(std::make_unique<UiListItem>(std::string_view {}, static_cast<int>(SpecialMenuEntry::None), UiFlags::ElementDisabled));
#ifndef USE_SDL1
		if (shownMenu == ShownMenuType::Settings && selectedCategory == &GetOptions().Controller
		    && HasNoneOf(GetOptions().Controller.virtualGamepad.GetFlags(), OptionEntryFlags::Invisible)) {
			vecDialogItems.push_back(std::make_unique<UiListItem>(_("Customize Layout"), static_cast<int>(SpecialMenuEntry::CustomizeGamepad), UiFlags::ColorUiGold));
			vecDialogItems.push_back(std::make_unique<UiListItem>(_("Reset Layout"), static_cast<int>(SpecialMenuEntry::ResetGamepad), UiFlags::ColorUiGold));
		}
#endif
