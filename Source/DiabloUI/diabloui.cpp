...
void UiFocus(std::size_t itemIndex, bool checkUp, bool ignoreItemsWraps = false, bool playSound = true)
{
...
SDL_Point EventPoint(const SDL_Event &event)
{
	if (event.type == SDL_EVENT_MOUSE_MOTION)
...
{
	const SDL_Point point = EventPoint(event);
	return SDLC_PointInRect(&point, &rect);
}

SDL_Rect InflateRect(SDL_Rect rect, int dx, int dy)
{
	rect.x -= dx;
	rect.y -= dy;
	rect.w += 2 * dx;
...
bool IsLeftMouseHeld(const SDL_Event &event)
{
	if (event.type == SDL_EVENT_MOUSE_MOTION)
...
void FocusFromScrollBarY(const UiScrollbar &uiSb, int y)
{
	if (SelectedItemMax == 0)
		return;
	const SDL_Rect bar = BarRect(uiSb);
	const int thumbH = uiSb.m_thumb.height();
	const int usable = std::max(1, bar.h - thumbH);
	const int rel = std::clamp(y - bar.y - thumbH / 2, 0, usable);
	const std::size_t index = static_cast<std::size_t>(rel) * SelectedItemMax / static_cast<std::size_t>(usable);
	UiFocus(index, /*checkUp=*/true, /*ignoreItemsWraps=*/true, /*playSound=*/false);
}
...
{
	if (!IsLeftMouseHeld(event) && event.type != SDL_EVENT_MOUSE_BUTTON_UP)
		return false;

	const SDL_Point point = EventPoint(event);

	if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
		if (point.y < uiList->m_rect.y || point.y >= uiList->m_rect.y + uiList->m_rect.h)
			return false;
		listDragState.tracking = true;
		listDragState.dragging = false;
		listDragState.startY = point.y;
		listDragState.startSelected = SelectedItem;
		uiList->Press(uiList->indexAt(point.y));
		return true;
	}

	if (event.type == SDL_EVENT_MOUSE_MOTION) {
		if (!listDragState.tracking || !IsLeftMouseHeld(event))
			return false;
		const int dy = point.y - listDragState.startY;
		if (std::abs(dy) < static_cast<int>(uiList->m_height) / 2 && !listDragState.dragging)
			return true;
		listDragState.dragging = true;
		const int itemDelta = -dy / static_cast<int>(uiList->m_height);
		const int unclamped = static_cast<int>(listDragState.startSelected) + itemDelta;
		const auto index = static_cast<std::size_t>(std::clamp(unclamped, 0, static_cast<int>(SelectedItemMax)));
		UiFocus(index, /*checkUp=*/true, /*ignoreItemsWraps=*/true, /*playSound=*/false);
		return true;
	}
	...
	if (listDragState.dragging) {
		listDragState.tracking = false;
		listDragState.dragging = false;
		return true;
	}
	listDragState.tracking = false;
	...
	std::size_t index = uiList->indexAt(point.y);
...
{
	const SDL_Point point = EventPoint(event);
	const SDL_Rect hitBar = InflateRect(uiSb->m_rect, 24, 8);
	...
	if (event.type == SDL_EVENT_MOUSE_BUTTON_UP && event.button.button == SDL_BUTTON_LEFT) {
		...
		if (scrollBarState.upArrowPressed && IsInsideRect(event, InflateRect(UpArrowRect(*uiSb), 12, 8))) {
			UiFocusUp();
			return true;
		}
		if (scrollBarState.downArrowPressed && IsInsideRect(event, InflateRect(DownArrowRect(*uiSb), 12, 8))) {
			UiFocusDown();
			return true;
		}
		return false;
	...
	if (event.type == SDL_EVENT_MOUSE_MOTION) {
		if (!scrollBarState.dragging || !IsLeftMouseHeld(event))
			return scrollBarState.dragging;
		FocusFromScrollBarY(*uiSb, point.y);
		return true;
	}

	if (event.type != SDL_EVENT_MOUSE_BUTTON_DOWN || event.button.button != SDL_BUTTON_LEFT)
		return false;
	if (!SDLC_PointInRect(&point, &hitBar) && !IsInsideRect(event, InflateRect(BarRect(*uiSb), 24, 8)))
		return false;

	if (IsInsideRect(event, InflateRect(UpArrowRect(*uiSb), 12, 8))) {
		scrollBarState.upArrowPressed = true;
		return true;
	}
	if (IsInsideRect(event, InflateRect(DownArrowRect(*uiSb), 12, 8))) {
...
{
	if (item->IsNotInteractive())
		return false;
	if (item->GetType() == UiType::Scrollbar) {
		return HandleMouseEventScrollBar(event, static_cast<UiScrollbar *>(item));
	}
	if (item->GetType() == UiType::List && (listDragState.tracking || listDragState.dragging)) {
		return HandleMouseEventList(event, static_cast<UiList *>(item));
	}
	if (!IsInsideRect(event, item->m_rect))
		return false;
	switch (item->GetType()) {
