...
void RunGameLoop(interface_mode uMsg)
{
	...
	nthread_ignore_mutex(false);
#ifdef __IPHONEOS__
	// UIKit import/document picker and audio interruptions can leave the
	// session paused with no keyboard P-key to resume.
...
[[noreturn]] void PrintHelpAndExit()
{
	...
	printNewlineInConsole();
#ifdef __IPHONEOS__
	printInConsole(_("The official website is https://nukehour.com"));
#else
...
{
#ifdef __IPHONEOS__
	*BufCopy(gszProductName, "Nuke Hour", " v", PROJECT_VERSION) = '\0';
#else
