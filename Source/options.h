...
enum class VirtualGamepadVisibility : uint8_t {
	Auto = 0,
	Always = 1,
	Hidden = 2,
};
...
struct ControllerOptions : OptionCategoryBase {
	...
	float fDeadzone;
	/** @brief When to show the on-screen virtual gamepad. */
	OptionEntryEnum<VirtualGamepadVisibility> virtualGamepad;
	bool virtualGamepadCustomLayout = false;
	float virtualGamepadPadX = 0;
	float virtualGamepadPadY = 0;
	float virtualGamepadActionsX = 0;
	float virtualGamepadActionsY = 0;
	float virtualGamepadMenuX = 0;
	float virtualGamepadMenuY = 0;
#ifdef __vita__
