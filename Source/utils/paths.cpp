...
#elif defined(__IPHONEOS__)
		configPath = FromSDL(IOSGetConfigPath());
#elif defined(NXDK)
...
{
	static std::optional<std::string> documentsPath;
	if (!documentsPath) {
		documentsPath = FromSDL(IOSGetDocumentsPath());
	}
...
#elif defined(__IPHONEOS__)
		// iOS packs game assets at the bundle root, not in an `assets/` subdirectory.
