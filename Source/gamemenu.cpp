...
void GamemenuContinue(bool bActivate);
...
#ifndef USE_SDL1
void GamemenuEditGamepad(bool bActivate);
#endif
...
#ifdef __IPHONEOS__
	{ GMENU_ENABLED, N_("Continue"),          &GamemenuContinue   },
#endif
	{ GMENU_ENABLED, N_("Options"),           &GamemenuOptions    },
#ifndef USE_SDL1
	{ GMENU_ENABLED, N_("Edit On-Screen Controls"), &GamemenuEditGamepad },
#endif
...
void GamemenuUpdateSingle()
{
	...
	for (TMenuItem *item = sgSingleMenu; item->fnMenu != nullptr; item++) {
		if (item->fnMenu == &gamemenu_save_game) {
			item->setEnabled(gbValidSaveFile);
		} else if (item->fnMenu == &GamemenuOptions) {
...
void GamemenuEditGamepad(bool bActivate)
{
	if (!bActivate)
		return;
	gamemenu_off();
	StartVirtualGamepadLayoutEditorFromGame();
}
#endif
