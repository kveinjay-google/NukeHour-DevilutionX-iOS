...
std::vector<std::string> GetMPQSearchPaths()
{
	...
	if (paths[0] == paths[1])
		paths.pop_back();
#ifdef __IPHONEOS__
	paths.push_back(paths::IOSDocumentsPath());
	if (paths.back() == paths::PrefPath() || paths.back() == paths::BasePath())
		paths.pop_back();
#endif
	paths.push_back(paths::ConfigPath());
	if (paths.size() >= 2) {
		const auto last = paths.end() - 1;
		if (std::find(paths.begin(), last, *last) != last)
			paths.pop_back();
	}

#if (defined(__unix__) || defined(__APPLE__)) && !defined(__ANDROID__) && !defined(__DJGPP__) && !defined(__IPHONEOS__)
...
#ifdef __IPHONEOS__
			while (!haveDiabdat && !haveSpawn) {
				if (!IOSPresentImportScreen(IOSImportGatePlayable)) {
...
#ifdef __IPHONEOS__
		while (!HasHellfireMpq) {
			if (!IOSPresentImportScreen(IOSImportGateHellfire)) {
				std::exit(EXIT_SUCCESS);
			}
			HasHellfireMpq = FindMPQ(GetMPQSearchPaths(), "hellfire");
		}
#else
#ifdef UNPACKED_MPQS
