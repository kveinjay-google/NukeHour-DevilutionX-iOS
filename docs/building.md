...
Or configure CMake yourself. Use `CMake/platforms/ios.toolchain.cmake` (lowercase `platforms`). Prefer Unix Makefiles over the Xcode generator with recent CMake/Xcode combinations.

Device (arm64):

```bash
cmake -S. -Bbuild-ios-device -DCMAKE_TOOLCHAIN_FILE=CMake/platforms/ios.toolchain.cmake -DENABLE_BITCODE=0 -DPLATFORM=OS64
cmake --build build-ios-device -j "$(sysctl -n hw.physicalcpu)" --config Release
```

Apple Silicon Simulator uses `SIMULATORARM64` (`SIMULATOR64` is Intel-only):

```bash
cmake -S. -Bbuild-ios-sim -DCMAKE_TOOLCHAIN_FILE=CMake/platforms/ios.toolchain.cmake -DENABLE_BITCODE=0 -DPLATFORM=SIMULATORARM64
...
rm -rf Payload
mkdir -p Payload
cp -R devilutionx.app Payload/
zip -r devilutionx-iOS.ipa Payload
```

See [docs/ios_build.md](ios_build.md) for signing and device install.
</details>
